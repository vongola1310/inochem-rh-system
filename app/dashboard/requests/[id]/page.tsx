...
import { cancelRequest, approveCancellation } from '@/app/actions/cancel-request'
import { 
  CalendarDays, Clock, AlertCircle, XCircle, RefreshCw, 
  CheckCircle, FileText, User, ArrowLeft, Calendar, Download 
} from 'lucide-react'
...
function calculateDaysBreakdown(startDate: Date, returnDate: Date | null, holidays: Set<string>) {
  ...
  const end = new Date(returnDate); end.setHours(0, 0, 0, 0)
...
export default async function RequestDetailPage({ params }: { params: { id: string } }) {
  ...
  const holidayDates = new Set(allHolidays.map(h => {
  ...
  const canDownload = (request.approvedByBoss || request.approvedByHR) && currentStatus !== 'CANCELLED'
  ...
  const getStatusInfo = () => {
    switch (currentStatus) {
      case 'APPROVED': return { label: 'Aprobado', color: 'text-[#73C056]', bg: 'bg-[#73C056]/10', border: 'border-[#73C056]/20', icon: CheckCircle }
      case 'REJECTED': return { label: 'Rechazado', color: 'text-red-600', bg: 'bg-red-50', border: 'border-red-200', icon: XCircle }
      case 'PENDING_BOSS': return { label: 'Firma de Jefe', color: 'text-amber-600', bg: 'bg-amber-50', border: 'border-amber-200', icon: Clock }
      case 'PENDING_HR': return { label: 'Validación RH', color: 'text-blue-600', bg: 'bg-blue-50', border: 'border-blue-200', icon: Clock }
      case 'CANCELLED': return { label: 'Cancelado', color: 'text-slate-500', bg: 'bg-slate-100', border: 'border-slate-200', icon: XCircle }
      case 'CANCELLATION_REQUESTED': return { label: 'Solicitó Cancelar', color: 'text-orange-600', bg: 'bg-orange-50', border: 'border-orange-200', icon: AlertCircle }
      default: return { label: currentStatus, color: 'text-slate-600', bg: 'bg-slate-50', border: 'border-slate-200', icon: AlertCircle }
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-slate-100 to-slate-50 p-4 sm:p-6 lg:p-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-6">
          <Link href="/">
            <Button variant="ghost" className="text-slate-600 hover:text-[#73C056] hover:bg-[#73C056]/5 transition-all -ml-2">
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden">
          ...
          <div className="p-6 sm:p-8 border-b border-slate-100 bg-slate-50/30">
            <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
              <div>
                ...
                <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 mb-1 flex items-center gap-2">
                  {isVacation ? <CalendarDays className="h-8 w-8 text-[#73C056]" /> : <Clock className="h-8 w-8 text-amber-500" />}
                ...
                <div className="flex items-center gap-2 text-slate-600 text-sm mt-2">
                  ...
                  <span className="text-slate-500">{request.user.jobTitle || 'Empleado'}</span>
                </div>
              </div>

              <div className="flex flex-col items-end gap-3">
                <Badge className={`text-sm px-4 py-1.5 font-semibold shadow-sm border ${statusInfo.bg} ${statusInfo.color} ${statusInfo.border}`}>
            ...
            {!canDownload && currentStatus !== 'REJECTED' && currentStatus !== 'CANCELLED' && (
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                  <div className="bg-white rounded-xl p-5 border border-slate-200 shadow-sm relative overflow-hidden group hover:border-blue-300 transition-colors">
                    ...
                    <p className="text-xs text-slate-500 uppercase tracking-wider mb-1 font-medium">Inicia</p>
                    <p className="text-xl font-bold text-slate-900 capitalize">
                      {format(request.startDate, "d 'de' MMMM", { locale: es })}
                    </p>
                    <p className="text-sm text-slate-400">{format(request.startDate, "EEEE, yyyy", { locale: es })}</p>
                  </div>

                  <div className="bg-white rounded-xl p-5 border border-slate-200 shadow-sm relative overflow-hidden group hover:border-blue-300 transition-colors">
                    <div className="absolute top-0 left-0 w-1 h-full bg-blue-400"></div>
                    <p className="text-xs text-slate-500 uppercase tracking-wider mb-1 font-medium">Regresa</p>
                    <p className="text-xl font-bold text-slate-900 capitalize">
                      {request.returnDate ? format(request.returnDate, "d 'de' MMMM", { locale: es }) : 'N/A'}
                    </p>
                    <p className="text-sm text-slate-400">{request.returnDate ? format(request.returnDate, "EEEE, yyyy", { locale: es }) : ''}</p>
                  </div>
                  ...
                  <div className="bg-[#73C056]/5 rounded-xl p-5 border border-[#73C056]/20 shadow-sm flex flex-col justify-center items-center text-center">
                    <p className="text-xs text-[#73C056] uppercase tracking-wider mb-1 font-bold">Días a descontar</p>
                    <div className="flex items-baseline gap-1">
                        <span className="text-4xl font-black text-[#73C056]">{request.daysRequested}</span>
                        <span className="text-sm font-medium text-[#73C056]">hábiles</span>
                    </div>
                  </div>
                </div>
                ...
                {breakdown && (
                  <div className="bg-slate-50 rounded-xl p-4 border border-slate-100">
                    <div className="grid grid-cols-3 gap-4 text-center">
                       <div><p className="text-xs text-slate-500 mb-1">Total días</p><p className="text-lg font-bold text-slate-800">{breakdown.total}</p></div>
                       <div className="border-x border-slate-200"><p className="text-xs text-slate-500 mb-1">Fines de Semana</p><p className="text-lg font-bold text-slate-600">{breakdown.weekends}</p></div>
                       <div><p className="text-xs text-slate-500 mb-1">Días Festivos</p><p className="text-lg font-bold text-slate-600">{breakdown.holidays}</p></div>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="bg-white rounded-xl p-5 border border-slate-200 shadow-sm relative overflow-hidden">
                        ...
                        <p className="text-xs text-slate-500 uppercase tracking-wider mb-1 font-medium">Fecha del Permiso</p>
                        <p className="text-xl font-bold text-slate-900 capitalize">{format(request.startDate, "EEEE, d 'de' MMMM", { locale: es })}</p>
                        <p className="text-sm text-slate-400">{format(request.startDate, "yyyy")}</p>
                    </div>
                    {request.permitTime && (
                        <div className="bg-white rounded-xl p-5 border border-slate-200 shadow-sm relative overflow-hidden">
                            <div className="absolute top-0 left-0 w-1 h-full bg-purple-400"></div>
                            <p className="text-xs text-slate-500 uppercase tracking-wider mb-1 font-medium">Horario Reportado</p>
                            <div className="flex items-center gap-2">
                                <Clock className="h-6 w-6 text-purple-500" />
                                <p className="text-2xl font-bold text-slate-900">{request.permitTime}</p>
                            </div>
                        </div>
                    )}
                </div>
                ...
                <div className="mt-4 flex items-start gap-2 text-xs text-slate-500 bg-slate-50 p-3 rounded-lg border border-slate-200">
                    ...
                    <p>Los permisos no descuentan saldo de vacaciones automáticamente, quedan a consideración de RH para nómina.</p>
                </div>
              </div>
            )}
          ...
          <CardFooter className="flex flex-col gap-3 border-t border-slate-100 p-6 bg-slate-50/30">
            ...
            {isOwner && currentStatus !== 'CANCELLED' && currentStatus !== 'REJECTED' && currentStatus !== 'CANCELLATION_REQUESTED' && (
                <form action={async (formData) => {
                    'use server'
                    await cancelRequest(formData)
                }} className="w-full">
                    <input type="hidden" name="requestId" value={request.id} />
                    <Button variant="destructive" className="w-full gap-2 h-12 text-base font-medium shadow-sm hover:bg-red-600 transition-all rounded-xl" type="submit">
                        <XCircle className="w-5 h-5"/>
                        {isApproved ? 'Solicitar Cancelación (Ya aprobada)' : 'Cancelar Solicitud'}
                    </Button>
                    {isApproved && <p className="text-xs text-center mt-3 text-slate-500">⚠️ Requiere aprobación de RH para devolver los días al saldo.</p>}
                </form>
            )}

            {/* 2. ACCIONES DE APROBACIÓN (SOLO SI TE TOCA) */}
            {showApprovalButtons && (
               <div className="flex flex-col sm:flex-row gap-3 w-full animate-in slide-in-from-bottom-2">
                 <form action={processRequest} className="flex-1">
                   <input type="hidden" name="requestId" value={request.id} />
                   <input type="hidden" name="action" value="REJECT" />
                   <Button variant="outline" className="w-full border-red-200 text-red-600 hover:bg-red-50 hover:border-red-300 h-12 rounded-xl font-medium" type="submit">
                     Rechazar Solicitud
                   </Button>
                 </form>
                 <form action={processRequest} className="flex-1">
                   ...
                   <Button className="w-full bg-[#73C056] hover:bg-[#62a847] text-white h-12 shadow-md font-bold rounded-xl transition-all hover:shadow-lg hover:-translate-y-0.5" type="submit">
            ...
            {isBossOrHR && isCancellationRequested && (
                <div className="w-full bg-orange-50 p-5 rounded-xl border border-orange-200 text-center shadow-sm">
                    <div className="flex justify-center mb-3"><div className="bg-orange-100 p-2 rounded-full"><AlertCircle className="h-6 w-6 text-orange-600"/></div></div>
                    <p className="text-sm text-orange-900 font-bold mb-4">El empleado solicita cancelar estas vacaciones ya aprobadas.</p>
                    <form action={async (formData) => {
                        'use server'
                        await approveCancellation(formData)
                    }}>
                        <input type="hidden" name="requestId" value={request.id} />
                        <Button className="bg-orange-500 hover:bg-orange-600 text-white w-full gap-2 h-12 shadow-sm rounded-xl font-bold" type="submit">
